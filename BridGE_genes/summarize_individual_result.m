function summarize_individual_result(ssmFile,R)
% get gene-gene interactions that have FDR<=0.25 or pv<=0.05

% ssmFile: ssM file name without disease model information, 
%          'ssM_mhygeSSI_alpha10.05_alpha20.05' or 'ssM_ld_mhygeSSI_alpha10.05_alpha20.05'
...
for i=1:length(model)
     load(sprintf('genstats_%s_%s_R%s.mat',ssmFile,model{i},num2str(R)))
...
result_protective_all = table(gene1,gene2,dmodel,msize,density,pv,fdr,fdr_both);
...
clear fdr density dmodel fdr_both 
...
result_risk_all = table(gene1,gene2,dmodel,msize,density,pv,fdr,fdr_both);
...
save(sprintf('summary_results_%s_R%s.mat',ssmFile,num2str(R)),'result_risk*','result_protective*')
