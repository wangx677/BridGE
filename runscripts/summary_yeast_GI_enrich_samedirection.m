...
for i=1:length(phenos)
     for j=1:length(models)
          ...
          if exist(filename,'file')==2
               data = readtable(sprintf('%s/GI_enrichment_samedirection_%s.xls',projectdir,models{j}));
          else
               data = '';
          end
...
saveas(gcf,sprintf('%s/results_collection/yeast/summary_yeast_GI_enrich_BPM_same_direction.png',getenv('BRIDGEPATH')))
saveas(gcf,sprintf('%s/results_collection/yeast/summary_yeast_GI_enrich_BPM_same_direction.pdf',getenv('BRIDGEPATH')))
...
ylabel(sprintf('# of enriched results (p<0.05) out of %s',num2str(size(GI_enrichment_t25_b50,1))));
legend({'t25-b50','t50-b25','t25-b25'})
saveas(gcf,sprintf('%s/results_collection/yeast/summary_yeast_GI_enrich_WPM_same_direction.png',getenv('BRIDGEPATH')))
saveas(gcf,sprintf('%s/results_collection/yeast/summary_yeast_GI_enrich_WPM_same_direction.pdf',getenv('BRIDGEPATH')))
