...
for i=1:length(phenos)
    ...
    for j=1:length(models)
          ...
          if isempty(data_bpm)~=1
               ...
               % keep BPMs that can be enriched in SGA (match direction)
               ind_bpm1 = find(ismember(data_bpm.eff_bpm,'risk') & min(tmp(:,[2 4 6]),[],2)<=0.05 & data_bpm.bpm_pv_discovery<=0.005);
               ind_bpm2 = find(ismember(data_bpm.eff_bpm,'protective') & min(tmp(:,[1 3 5]),[],2)<=0.05 & data_bpm.bpm_pv_discovery<=0.005);
               ind_bpm = union(ind_bpm1,ind_bpm2);
          ...
          if isempty(data_wpm)~=1
               ...
               % keep WPMs that can be enriched in SGA (match direction)
               ind_wpm1 = find(ismember(data_wpm.eff_wpm,'risk') & min(tmp(:,[2 4 6]),[],2)<=0.05 & data_wpm.wpm_pv_discovery<=0.05);
               ind_wpm2 = find(ismember(data_wpm.eff_wpm,'protective') & min(tmp(:,[1 3 5]),[],2)<=0.05 & data_wpm.wpm_pv_discovery<=0.05);
               ind_wpm = union(ind_wpm1,ind_wpm2);
...
%     writetable(results{i},sprintf('/project/csbio/wwang/BridGE/results_collection/yeast/results_high_confidence_%s_samedirection.xls',binarization{m}),'Sheet',i);
...
writetable(results_all,sprintf('/project/csbio/wwang/BridGE/results_collection/yeast/results_high_confidence_%s_samedirection.xls',binarization{m}));
writetable(results_all,sprintf('/project/csbio/wwang/BridGE/results_collection/yeast/results_high_confidence_%s_samedirection.csv',binarization{m}),'Delimiter',';');
