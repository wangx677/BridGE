function ssM = rrssiwithin(data,pheno)
...
% compute pairwise relative risk for 00
x0 = control0'*control0+1;
x1 = case0'*case0+1;
n0 = length(x)+1;
n1 = length(y)+1;
rr00{1} = (x0/n0)./(x1/n1);
...
tmp2 = ((n1-x1)./x1)./n1;
for tt=1:2
     rr00ci{tt} = log(rr00{tt})-1.96*sqrt(tmp1+tmp2);
     rr00ci{tt} = exp(rr00ci{tt});
end

% compute pairwise relative risk for 01
x0 = control0'*control1+1;
x1 = case0'*case1+1;
n0 = length(x)+1;
n1 = length(y)+1;
rr01{1} = (x0/n0)./(x1/n1);
...
tmp2 = ((n1-x1)./x1)./n1;
for tt=1:2
     rr01ci{tt} = log(rr01{tt})-1.96*sqrt(tmp1+tmp2);
     rr01ci{tt} = exp(rr01ci{tt});
end

% compute pairwise relative risk for 10
x0 = control1'*control0+1;
x1 = case1'*case0+1;
n0 = length(x)+1;
n1 = length(y)+1;
rr10{1} = (x0/n0)./(x1/n1);
...
tmp2 = ((n1-x1)./x1)./n1;
for tt=1:2
     rr10ci{tt} = log(rr10{tt})-1.96*sqrt(tmp1+tmp2);
     rr10ci{tt} = exp(rr10ci{tt});
end

% compute pairwise relative risk for 11
x0 = control1'*control1+1;
x1 = case1'*case1+1;
n0 = length(x)+1;
n1 = length(y)+1;
rr11{1} = (x0/n0)./(x1/n1);
...
tmp2 = ((n1-x1)./x1)./n1;
for tt=1:2
     rr11ci{tt} = log(rr11{tt})-1.96*sqrt(tmp1+tmp2);
     rr11ci{tt} = exp(rr11ci{tt});
end

% compute the slope of  [max(rr00,rr01,rr10),rr11], [1 2]
for tt=1:2
     rrmax{tt} = max(rr00{tt},max(rr01{tt},rr10{tt}));
     slope{tt} = rr11{tt}-rrmax{tt};
     slope{tt} = tril(slope{tt},-1) + triu(slope{tt},1);
     slope{tt}(rr11ci{tt}<1) = 0;
end
