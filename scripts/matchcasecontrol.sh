...
# size2 clustering
plink1.9 --bfile ${plinkFile} --read-genome ${plinkFile}.genome --cluster --cc --mc 2 --ppc 0.01 --noweb --allow-no-sex --out ${plinkFile} > /dev/null
...
plink1.9 --bfile ${plinkFile} --allow-no-sex --keep cluster2_subject2keep --make-bed --out ${outputFile}  > /dev/null
