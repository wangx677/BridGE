function generate_BPM_WPM_info(projectdir,outputxls,bpmindfile,models,snppathwayfile,snpgenemappingfile)
...
cd(projectdir)
     ...
     ssmfile = sprintf('ssM_hygeSSI_alpha10.05_alpha20.05_%s_R0.mat',models);
     try
          data = readtable(outputxls,'Sheet',3);
     catch
          data = [];
     end
     ...
     if isempty(data)~=1
          if nnz(data.fdrBPM<=0.25)>0
               ...
               for j=1:size(data,1)
                    outputfile=sprintf('%s/BPM%s_%s_%s_fdr%.2f_pv%.2e_rs%.2f',outputdir,num2str(j),models,data.eff_bpm{j},data.fdrBPM(j),data.bpm_pv_discovery(j),data.bpm_ranksum_discovery(j));
     ...
     try          
          data = readtable(outputxls,'Sheet',4);
     catch
          data = [];
     end
     ...
     if isempty(data)~=1
          if nnz(data.fdrWPM<=0.25)>0
               ...
               for j=1:size(data,1)
                    outputfile=sprintf('%s/WPM%s_%s_%s_fdr%.2f_pv%.2e_rs%.2f',outputdir,num2str(j),models,data.eff_wpm{j},data.fdrWPM(j),data.wpm_pv_discovery(j),data.wpm_ranksum_discovery(j));
