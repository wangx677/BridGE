function [BPM_nosig_noRD,WPM_nosig_noRD,PATH_nosig_noRD,BPM_group,WPM_group,PATH_group] = check_redundancy(fdrBPM,fdrWPM,fdrPATH,bpmindFile,fdrcut)
...
for fdrcut=0.05:0.05:fdrcut
     ...
     if (length(ind1)>1)
          BPM_sim{1} = bpmsim(BPM.ind1(ind1),BPM.ind2(ind1),BPM.ind1(ind1),BPM.ind2(ind1),2);
          TTT = BPM_sim{1}>=0.25;
     ...
     if (length(ind2)>1)
          BPM_sim{2} = bpmsim(BPM.ind1(ind2),BPM.ind2(ind2),BPM.ind1(ind2),BPM.ind2(ind2),2);
          TTT = BPM_sim{2}>=0.25;
     ...
     if (length(ind3)>1)
          BPM_sim{3} = bpmsim(WPM.ind(ind3),WPM.ind(ind3),WPM.ind(ind3),WPM.ind(ind3),2);
          TTT = BPM_sim{3}>=0.25;
     ...
     if (length(ind4)>1)
          BPM_sim{4} = bpmsim(WPM.ind(ind4),WPM.ind(ind4),WPM.ind(ind4),WPM.ind(ind4),2);
          TTT = BPM_sim{4}>=0.25;
